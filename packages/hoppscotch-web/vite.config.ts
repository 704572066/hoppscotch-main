...
import Pages from "vite-plugin-pages" // 能够将文件系统生成对应的路由，从而省去手动配置路由的时间。
import Layouts from "vite-plugin-vue-layouts" // 通过 vite-plugin-pages 自动生成的路由，都是一级路由。而实际项目开发中，我们会使用嵌套路由的特性，该插件把通过 vite-plugin-pages 生成的一级路由处理成嵌套路由
